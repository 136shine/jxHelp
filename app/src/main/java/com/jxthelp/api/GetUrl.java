package com.jxthelp.api;

/**
 * Created by idisfkj on 15/10/18.
 * Email : idisfkj@qq.com.
 */
public class GetUrl {
    public static String LGUrl="http://www.jxust.cn/list/10";
    public static String XYUrl="http://www.jxust.cn/list/11";
    public static String MTUrl="http://www.jxust.cn/list/14";
    public static String XSUrl="http://www.jxust.cn/list/16";
    public static String ImageUrl="http://www.jxust.cn";
    public static String XXUrl="http://www.jxust.cn/list/12";
    public static String ZBUrl="http://www.jxust.cn/list/13";
    public static String ZPUrl="http://www.jxust.cn/list/17";
    public static String WEATHERUrl1="http://api.map.baidu.com/telematics/v3/weather?location=";
    public static String WEATHERUrl2="&output=xml&ak=eT3fBLMwyKuAMebeQUHUCOi6";
    public static String Url1 = "http://jw.jxust.cn/default2.aspx";
    public static String Url2="http://jw.jxust.cn/jstjkbcx.aspx?zgh=";
    public static String Url3 = "http://jw.jxust.cn/js_main.aspx?xh=";
    public static String PersonUrl="http://jw.jxust.cn/js_grjl.aspx?zgh=";
    public static String TestUrl="http://jw.jxust.cn/js_ksap.aspx?zgh=";
}

